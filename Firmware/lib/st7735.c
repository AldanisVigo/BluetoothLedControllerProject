 ...
 * @return void
*/
void ST7735_DrawCircle(struct st7735 * lcd, uint8_t x, uint8_t y, uint8_t radius){
  circleBres(lcd, x, y, radius);
}

// Function to put pixels
// at subsequence points
void drawCircle(struct st7735 * lcd,int xc, int yc, int x, int y)
{
    ST7735_DrawPixel(lcd,xc+x, yc+y, RED);
    ST7735_DrawPixel(lcd,xc-x, yc+y, RED);
    ST7735_DrawPixel(lcd,xc+x, yc-y, RED);
    ST7735_DrawPixel(lcd,xc-x, yc-y, RED);
    ST7735_DrawPixel(lcd,xc+y, yc+x, RED);
    ST7735_DrawPixel(lcd,xc-y, yc+x, RED);
    ST7735_DrawPixel(lcd,xc+y, yc-x, RED);
    ST7735_DrawPixel(lcd,xc-y, yc-x, RED);   
}
...
void circleBres(struct st7735 * lcd, int xc, int yc, int r)
{
    int x = 0, y = r;
    int d = 3 - 2 * r;
    drawCircle(lcd,xc, yc, x, y);
    while (y >= x)
    {
